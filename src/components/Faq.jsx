                ...
                <div className="max-w-[1022px] flex flex-col gap-6 mx-auto">
                    {FAQ_DATA_LIST.map((item, index) => (
                        <div className={`rounded-xl  ${active === index ? 'bg-custom p-[1px]' : ''}`}>
                            <div key={index} className={`rounded-xl !bg-wood-smoke border gap-4 px-4 sm:px-6 border-solid border-dark-blue transition-all ease-linear duration-500 ${active === index ? '!border-none' : 'pb-4'}`}>
                                ...
                                <p className={`text-white max-md:text-sm overflow-hidden transition-all ease-linear duration-500  ${active === index ? "max-h-36 max-sm:max-h-56 pb-4" : "max-h-0"}`} >{item.subHeading}</p>
                            </div>
                        </div>
                    ))}
                </div>
